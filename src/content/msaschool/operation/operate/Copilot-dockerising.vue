...
- 이후, 전체 내용을 선택 > (Cmd+L, Ctrl+Shift+L)을 눌러 코파일럿 창에 입력한 후 다음의 프롬프트를 추가합니다.
```
prompt : 위 default 프로파일을 복제해서, 'docker' 프로파일을 default 프로파일 아래에 추가해 줘.
```
- 생성 결과를 application.yml에 반영하고, 추가된 docker 프로파일의 Kafka broker endpoint를 'my-kafka:9092'로 수정되도록 추가 프롬프팅을 진행합니다.
```
...
```
- 생성 결과를 Dockerfile에 반영하고, 정합성 있는 Dockerfile이 완성되도록 추가로 프롬프팅을 진행합니다.
```
...
- 현재 프로젝트는 메이븐 빌드 환경이므로, 메이븐 타스크를 추가를 위해 다음과 같이 프롬프팅 합니다.
```
prompt : 여기에 maven package 스텝을 추가하되, 빌드시 테스트는 Skip하도록 해줘
```

- 완성된 github-action.yml을 인스펙션합니다.
